...
import { Plus, Search, Pencil, Trash2, ImageOff, Star, Loader2 } from 'lucide-react'
import { toast } from 'sonner'

const emptyItem = {
  name_en: '', name_ar: '', desc_en: '', desc_ar: '',
  category_id: null, price: 0, image_url: '',
  has_combo: false, combo_price: 0, combo_desc_en: '', combo_desc_ar: '',
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading && <tr><td colSpan={7} className="px-6 py-12 text-center"><Loader2 className="w-5 h-5 animate-spin mx-auto text-orange-500" /></td></tr>}
              {!loading && filtered.length === 0 && <tr><td colSpan={7} className="px-6 py-12 text-center text-muted-foreground">No items</td></tr>}
              {filtered.map((it, idx) => (
                <motion.tr
                  key={it.id}
                ...
                >
                  <td className="px-6 py-3">
                    {it.image_url ? (
                      <img src={it.image_url} alt="" className="w-10 h-10 rounded-lg object-cover" onError={(e)=>{e.target.style.display='none'}} />
                    ) : (
                      <div className="w-10 h-10 rounded-lg bg-secondary flex items-center justify-center"><ImageOff className="w-4 h-4 text-muted-foreground" /></div>
                    )}
                  </td>
                  <td className="px-6 py-3">
                  ...
                  <td className="px-6 py-3 font-mono font-semibold">${Number(it.price).toFixed(2)}</td>
                  <td className="px-6 py-3"><span className="inline-flex items-center gap-1 font-mono text-xs"><Star className="w-3 h-3 text-orange-500 fill-orange-500" />{Number(it.rating || 0).toFixed(1)}</span></td>
                  <td className="px-6 py-3">
                    <Switch checked={it.available} onCheckedChange={() => toggleAvail(it)} className="data-[state=checked]:bg-orange-500" />
                  </td>
                  <td className="px-6 py-3 text-right">
                    <div className="inline-flex gap-1">
                      <Button size="icon" variant="ghost" onClick={() => openEdit(it)} className="h-8 w-8"><Pencil className="w-3.5 h-3.5" /></Button>
                      <Button size="icon" variant="ghost" onClick={() => remove(it.id)} className="h-8 w-8 hover:text-destructive"><Trash2 className="w-3.5 h-3.5" /></Button>
                    </div>
                  </td>
                </motion.tr>
              ))}
      ...
      <Dialog open={modal} onOpenChange={setModal}>
        <DialogContent className="max-w-2xl bg-card border-border max-h-[90vh] overflow-y-auto scrollbar-thin">
          <DialogHeader>
            <DialogTitle className="font-display text-xl">{editing ? 'Edit Item' : 'Add New Item'}</DialogTitle>
          </DialogHeader>
